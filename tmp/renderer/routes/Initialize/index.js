  ...
  renderMessage = (status) => {
    switch (status) {
      ...
      default: {
        return (
          <div className={styles.hintArea}>
            <h3 dangerouslySetInnerHTML={{ __html: localizedStrings.initing }}></h3>
      ...
      case Status.error: {
        return (
          <FlatButton
            ...
            secondary
            onTouchTap={() => setTimeout(this.handleInitialize, 300)}
          />
        );
      }
    ...
    return (
      <div className={styles.container}>
        ...
        <div className={styles.upper}>
          {
            Array.from(Array(deviceNum)).map((__, index) => this.renderCard(index))
          }
        </div>
      </div>
    );
