...
class ImportMemo extends React.Component {
  state = {
  ...
  handleimportmemos = () => {
    ...
    this.props.dispatch({
      ...
      payload: { Memos },
      onComplete: this.handleimportmemosCallback,
    });
  }

  handleimportmemosCallback = (error) => {
    console.log('onComplete')
    if (error) {
      if (error == 0x80000008) {
        this.setState({ initStatus: Status.cancel });
      } else {
  ...
  renderMessage = (status) => {
    switch (status) {
      case Status.error: {
        return (
          <div className={styles.hintArea}>
            <h3>{localizedStrings.importmemoError}</h3>
            <h4>{localizedStrings.importmemoErrorTips}</h4>
          </div>
        );
      }
      ...
      case Status.success: {
        return (
          <div className={styles.hintArea}>
            <h3>{localizedStrings.importmemoSuccess}</h3>
          </div>
        );
      }
      ...
      case Status.cancel: {
        return (
          <div className={styles.hintArea}>
            <h3>{localizedStrings.importmemo}</h3>
            <h4>{localizedStrings.importmemoTips}</h4>
          </div>
        );
      }
      ...
      default: {
        return (
          <div className={styles.hintArea}>
            <h3 dangerouslySetInnerHTML={{ __html: localizedStrings.importmemoIng }}></h3>
            <h4>{localizedStrings.importmemoIngTips}</h4>
          </div>
        );
      }
    }
      ...
      return (
        ...
        <div>
          ...
          <FlatButton
            ...
            labelStyle={{ fontSize: 20 }}
            primary
            onTouchTap={this.handleimportmemos}
          />
        </div>
    )
  ...
  render() {
    const { deviceNum, Memos } = this.props;
...
export default connect(mapStateToProps)(ImportMemo);
