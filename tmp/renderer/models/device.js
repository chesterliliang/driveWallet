...
export default {
  ...
  effects: {
    * tryConnect({
      ...
      for (let index = 0; index < deviceNum; index += 1) {
        ...
        yield put({ type: 'data/clientCoreInvoke', payload: { category: 'driver', fn: 'getStatusByIndex', args: [index] } });
    ...
    * getAddresses(action, { put, take, call }) {
      ...
      const { type: actionType, payload: deviceList } = yield take(['device/checkStatusSuccess', 'device/checkStatusError']);
      if (actionType === 'device/checkStatusSuccess') {
        const initializedCount =
          deviceList.filter(({ SesStatus: initializing }) => !initializing).length;

        if (initializedCount === deviceList.length) {
          yield put({ type: 'data/clientCoreInvoke', payload: { category: 'driver', fn: 'getDeviceAddresses', args: [] } });
    ...
    * format(action, { put, take }) {
      ...
      yield put({ type: 'data/clientCoreInvoke', payload: { category: 'driver', fn: 'format', args: [] } });
      const { type: actionType, payload: result } = yield take(['device/formatSuccess', 'device/formatError']);
    ...
    * changepin(action, { put, take }) {
    window.showLoading();
    yield put({ type: 'send/setInProgress', payload: true });
    yield put({ type: 'app/setState', payload: { locked: true } });
    console.log('[device][changePIN] enter')
    yield put({ type: 'data/clientCoreInvoke', payload: { category: 'driver', fn: 'changePIN', args: [0] } });
    const { type: actionType, payload: result } = yield take(['device/changePINSuccess', 'device/changePINError']);
    yield put({ type: 'app/setState', payload: { locked: false } });
    window.dismissLoading();
    yield put({ type: 'send/setInProgress', payload: false });
    ...
    * importMNE({ payload: { Memos }, onComplete }, { put, select, take, fork, call }) {
    ...
    if (actionType !== 'device/checkStatusSuccess') {
      yield fork(assertPinStatus, { put, take, call }, { type: 'device/importMNE', payload: { Memos }, onComplete });
      return;
    }

    yield put({ type: 'data/clientCoreInvoke', payload: { category: 'driver', fn: 'importMne', args: [Memos] } });//index=0,seedlen=32，实际seedlen可选16,20,24,28,32修改此参数将影响助记词长度

    let { type: initActionType, payload: initResult } = yield take(['device/importMneSuccess', 'device/importMneError']);
    if (initActionType === 'device/importMneSuccess' && !initResult && onComplete) {
      onComplete(null);
    } else if (onComplete) {
    ...
    * initialize({ payload: { threshold }, onComplete }, { put, select, take }) {
      // const { num: deviceNum } = yield select(state => state.device);

      yield put({ type: 'data/clientCoreInvoke', payload: { category: 'driver', fn: 'initialize', args: [0, 32] } });//index=0,seedlen=32，实际seedlen可选16,20,24,28,32修改此参数将影响助记词长度

      const { type: initActionType, payload: initResult } = yield take(['device/initSuccess', 'device/initError']);
      if (initActionType === 'device/initSuccess' && !initResult && onComplete) {
        onComplete(null);
      } else if (onComplete) {
        onComplete(initResult);
      }
    ...
    * handleStatus({
      ...
      switch (decimalToHexString(status)) {
        ...
        case '80000007': {
          ...
          if (!statusToastId && outloading) {
            statusToastId = 'fakeHoldId';
            statusToastId = window.showToast(localizedStrings.unlockToast, null, false, false);
          }
          if(outloading)
              yield put({ type: 'app/setState', payload: { locked: true } });
          break;
        ...
        case '80000001': {
          ...
          if (!statusToastId) {
            statusToastId = 'fakeHoldId';
            statusToastId = window.showToast(localizedStrings.reconnectToast, null, false, false);
          }
          yield put({ type: 'app/setState', payload: { locked: true } });
          break;
        }
  ...
  reducers: {
    setNumber(state, { payload: { DevNum: num } }) {
      if (num < 1) {
        return { num: 0, list: [] };
      }
    ...
    setAppVer(state, { payload: { ElectronVer, MidWareVer } }) {
      return {
        ...state,
        ElectronVer,
        MidWareVer,
    ...
    setloopstop(state, { payload: { insetting } }) {
      return {
        ...state,
        insetting,
      };
    ...
    setLcdState(state, { payload: { lcdState} }) {
      return {
        ...state,
        LcdState: lcdState,
      };
    ...
    addDevice(state, { payload: { index, device } }) {
      ...
      if (index >= oldList.length) {
        ...
        newList = oldList.map((value, tmpIndex) => (tmpIndex === index ? device : value));
      }
    ...
    checkStatusSuccess(state, { payload: [{ COS }] }) {
      return {
        ...state,
        COSVersion: COS,
        LcdState: 0,
