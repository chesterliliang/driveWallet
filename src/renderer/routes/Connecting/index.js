...
class Connecting extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps &&
      nextProps.deviceNum !== this.props.deviceNum &&
      nextProps.deviceNum > 0
    ) {
      this.props.dispatch({
        type: 'device/tryConnect',
        payload: { delay: 1000 },
    ...
    if (
      ...
      const { code, data } = this.checkDevices(
        nextProps.deviceNum,
  ...
  handleRefresh = () => {
    ...
    this.props.dispatch({
      type: 'data/clientCoreInvoke',
      payload: { category: 'driver', fn: 'getDevNum', args: [] },
    });
  };
  ...
  checkDevices = (deviceNum, deviceList) => {
    if (
      this.props.LcdState &&
      this.props.LcdState != 1 &&
      this.props.LcdState != 0x400
    ) {
    ...
    const groupKeys = deviceList
      .map(({ groupKey }) => groupKey)
      .reduce(
        (tmp, groupKey) =>
          tmp.indexOf(groupKey) > -1 ? tmp : [...tmp, groupKey],
        []
      );
    ...
    const groupMembers = groupKeys.reduce((tmp, groupKey) => {
      return {
        ...tmp,
        [groupKey]: deviceList.filter(
          ({ groupKey: tmpKey }) => tmpKey === groupKey
        ),
      };
    }, {});

    const groupMemberNums = groupKeys.map(
      groupKey => groupMembers[groupKey].length
    );
    ...
    return {
      ...
      data: { initialized: initializedCount > 0 },
    };
  };
      ...
      case ErrorCode.noError: {
        ...
        return (
          <div className={styles.hintArea}>
            ...
            <h4
              dangerouslySetInnerHTML={{ __html: localizedStrings.readyHint }}
            />
          </div>
        );
      }
        ...
        return (
          <div>
            <FlatButton
              ...
              labelStyle={{ fontSize: 20 }}
              primary
              onClick={this.handleNext.bind(this, data.initialized)}
            />
            <FlatButton
  ...
  render() {
    ...
    if (deviceNum < 1) {
      upper = (
        ...
        >
          <h4 style={{ color: 'white', fontSize: '28px', marginBottom: 20 }}>
      ...
      upper = Array.from(Array(deviceNum)).map((__, index) =>
        this.renderCard(index, deviceList, code && { code, data })
      );
    }
    ...
    return (
      <div className={styles.container}>
        {((window.location &&
          window.location.href.includes('connecting') &&
          !this.state.COSPath) ||
...
function mapStateToProps({
  device: { num: deviceNum, list, addresses, COSVersion, LcdState },
}) {
