...
import { delFile } from './autoUpdate';

let timerID = null;
let burning = false;

function checkCOSVersion(timeout) {
...
async function getUpdateInfo() {
  let url =
    'https://nebuladownload.oss-cn-beijing.aliyuncs.com/wookong/personal/COS/latest-cos.json';

  //removeIf(!consoleShow)
  url =
    'https://nebuladownload.oss-cn-beijing.aliyuncs.com/wookong/personal/COS_TEST/latest-cos.json';
...
async function checkDownloaded(cosVersion) {
  ...
  if (intnewVersion > intcosVersion) {
    global.cosUpdateInfo = {
      newVersion: true,
      version: `${parseInt('newVersion'.slice(0, 2), 16)}.${parseInt(
        'newVersion'.slice(2, 4),
        16
      )}.${parseInt('newVersion'.slice(4, 6), 16)}.${parseInt(
        'newVersion'.slice(6, 8),
        16
      )}`,
    };
  } else {
...
async function burn(COSPath) {
  ...
  try {
    ...
    if (!resultCode) {
      ...
      // 刷新软件
      await global.services.core.driver.getStatus();
...
async function update(
  ...
  clientNotUpdate = false
) {
  if (timerID) {
    return;
  }

  const cosVersion = await checkCOSVersion(timeout);
  ...
  if (existedCOS && optionalUpdate) {
    if (clientNotUpdate) return false;
    const win = window
      .getWindows()
      .slice(-1)
      .pop();
  ...
  if (appVersion >= minAppVersion && intnewVersion > intcosVersion) {
    console.log(
      `global.localizedStrings.updateCOSTitle = ${
        global.localizedStrings.updateCOSTitle
      }`
    );
    console.log(
      `global.localizedStrings.updateCOSMessage = ${
        global.localizedStrings.updateCOSMessage
      }`
    );
    global.cosUpdateInfo = {
    ...
    if (clientNotUpdate) return true;
    if (optionalUpdate) {
      dialog.showMessageBox({
        type: 'info',
        buttons: [global.localizedStrings.OK],
        title: global.localizedStrings.updateCosTitle,
        message: global.localizedStrings.latestCOSVersion,
      });
    }
  }
}

const checkCOSUpdate = async timeout => {
  const cosVersion = await checkCOSVersion(timeout);
  const { cosVersion: newVersion } = await getUpdateInfo();
  const intnewVersion = parseInt(newVersion.slice(4, 8), 16);
  const intcosVersion = cosVersion[3] + 16 * cosVersion[2];
  if (intnewVersion > intcosVersion) {
    global.cosUpdateInfo = {
...
export { update, checkDownloaded, burn, checkCOSUpdate };
