...
import {
  GetCoinAddress as getDeviceAddresses,
} from './GetCoinAddress';
...
import { keyConfirmAddress } from './CoinsOperate';
import verifyPkgSignature from './VerifyPkgSignature';
...
async function getStatus(handleType) {
  try {
    ...
    if (newPinStatus !== pinstatus) {
      ...
      statusCallback && statusCallback(null, newPinStatus, handleType);
    }
    ...
    const {
      DevInfo: [{ COS, LcdState }],
    } = data;
    COSVersion = COS;
    if (!isOldDevice) {
      if (
        LcdState != PAEW_DEV_INFO_LCD_NULL &&
        LcdState != PAEW_DEV_INFO_LCD_SHOWLOGO &&
        LcdState != PAEW_DEV_INFO_LCD_VERIFYPIN
      ) {
        statusCallback && statusCallback(null, LcdState, handleType);
        return LcdState;
      }
    } else {
    ...
    statusCallback && statusCallback(error, null, handleType);
...
async function getAppVersion() {
  const libVer = await GetLibVer();
  console.log('libVer', libVer);
  if (libVer && libVer.result !== 0) {
    console.log('getAppVersion errors: ', libVer.result);
    return { result: -1 };
  }
  return {
    result: 0,
    ElectronVer: app.getVersion(),
    MidWareVer: libVer.libVer,
  };
}
