...
*/
const BTCTXSign = async (utxos, currentTx) => {
  console.log('[Nebula Driver][PAEW_BTCTXSign]: Start')
  ...
  utxos.forEach(function (value, index) {
    ...
    var ppbTXSig = ucharArray(PAEW_BTC_SIG_MAX_LEN)
    ppbTXSigArray.push(ppbTXSig)
    pnTXSigLenArray.push(PAEW_BTC_SIG_MAX_LEN)
  })
  ...
  console.log(`[Nebula Driver][BTCTXSign][InitContext]result=${result},counter=${devCounter.deref()}`)
  if (result !== 0) {
    return {result}
  }

  try {
  result = await Api.PAEW_DeriveTradeAddress(ppPAEWContext.deref(),0 , 0, puiBTCDerivePath, uiBTCDerivePathLen.deref())//index = 0,cointype = 0(bitcoin)
  console.log(`[Nebula Driver][BTCTXSign][PAEW_DeriveTradeAddress]result=${result}`)
  if (result !== 0) {
    return {result}
  }
  ...
  var strSigns = []
    ...
    strSigns = await new Promise((resolve, reject) => {
      Api.PAEW_BTC_TXSign.async(ppPAEWContext.deref(), 0, nUTXOCount, ppbUTXOs, pnUTXOLen, pbCurrentTX, pbCurrentTXLen, ppbTXSigs, pnTXSigLen, (err, res) => {
        console.log(`[Nebula Driver][BTCTXSign][BTC_TXSign]result=${res}`)
        ...
        for (var i = 0; i < nUTXOCount; i++) {
          ...
          console.log(`[Nebula Driver][BTCTXSign][BTC_TXSign]ppbTXSigs[${i}]=${strSign}`)
          strs.push(strSign)
        }
    ...
    result = 0
    return { result, strSigns, pnTXSigLen }
  } catch (err) {
    return {result:err}
  }
  finally{
    result = await Api.PAEW_FreeContext(ppPAEWContext.deref())
    console.log(`[Nebula Driver][BTCTXSign][FreeContext]result=${result}`)
    console.log('[Nebula Driver][PAEW_BTCTXSign]: End')
  }
}
