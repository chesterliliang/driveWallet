...
const composeTx = async ({ inputs, outputs, testNet }) => {
  ...
  if (inputs[0].signature) {
    inputs.forEach((e, i) =>
      txRaw.setInputScript(i, Buffer.from(e.signature, 'hex'))
    );
  }
...
const getBalance = async ({ address, testNet, workNodeIndex = 0 }) => {
  try {
    console.log('btcNode111', config.node.BTC);
    ...
    logger.error(
      ...
      e,
      ['btc getBalance error'],
      address
    );

    BTCWorkNodeIndex = workNodeIndex;
    if (workNodeIndex <= config.node.BTC.WorkNodes.length - 1) {
      Object.assign(config.node.BTC, {
        MainNet: config.node.BTC.WorkNodes[workNodeIndex],
      });
      return await getBalance({
        address,
        testNet,
        workNodeIndex: ++workNodeIndex,
      });
    } else if (workNodeIndex === config.node.BTC.WorkNodes.length) {
      Object.assign(config.node.BTC, {
        MainNet: config.originNode.BTC.MainNet,
      });
      return await getBalance({ address, testNet });
    }
    BTCWorkNodeIndex = 0;
...
const prepareTx = async ({ address, to, value, fee, selector, testNet }) => {
  ...
  if (
    !balance.minus(value).greaterThanOrEqualTo(0) &&
    new BigNumber(value).dividedBy(Rate) != -1
  )
    throw new Error(20304);

  let utxos = await getAddressUtxo({ address, testNet });
  utxos = _.filter(
    utxos,
    e => e.confirmations >= config.coins.BTC.MinimalConfirmation
  );
  ...
  if (fee) {
    ...
    if (new BigNumber(value).dividedBy(Rate) == -1) {
    ...
    if (new BigNumber(value).dividedBy(Rate) == -1) {
      for (let i = 0; i < utxos.length; i++) {
        ...
        txTotalBytesNum =
          txGeneralBytesNum +
          txOutputBytesNum +
          txInputUnitBytesNum * inputs.length;
        calculatedFee = feePrice
          .times(txTotalBytesNum)
          .dividedBy(1024)
          .floor();
      }
      fee = calculatedFee;
      logger.log(
        `inputs value=${sum},outputs value=${value},change=${reminder},fee=${calculatedFee},tx bytes=${txTotalBytesNum}, fee price=${feePrice}`,
        ['Prepare Tx']
      );
    } else {
      for (let i = 0; i < utxos.length; i++) {
  ...
  if (result) {
    outputs = JSON.parse(`[{"address": "${to}", "amount": ${value}}]`);
    logger.log(`outputs=${JSON.stringify(outputs)}`, ['Prepare Tx']);
  } else {
    outputs = JSON.parse(
      `[{"address": "${to}", "amount": ${value}}, {"address": "${address}", "amount": ${reminder}}]`
    );
    logger.log(`outputs=${JSON.stringify(outputs)}`, ['Prepare Tx']);
  }
  logger.log(
    `to: ${to}, value: ${value}, address: ${address}, reminder: ${reminder}`,
    ['Prepare Tx']
  );
  logger.log(
    `inputs=${JSON.stringify(inputs)},outputs=${JSON.stringify(outputs)}`,
    ['Prepare Tx']
  );
...
const signTx = async ({ address, to, value, fee, selector, memo, testNet }) => {
  ...
  try {
    ...
    for (let i = 0; i < inputs.length; i++) {
      preTxs.push(
        (await getRawTransaction({ txid: inputs[i].txid, testNet })).rawtx
      );
    }
    ...
    if (!!(result.result || parseInt(result, 10))) {
      logger.error(
        `signData=${JSON.stringify({ preTxs, rawTx: tx.rawTx })}`,
        result,
        ['BTCTXSign error'],
        address
      );
      return {
        result: result.result || parseInt(result, 10),
        keyResult: -1,
    ...
    if (BTCWorkNodeIndex <= config.node.BTC.WorkNodes.length - 1) {
      Object.assign(config.node.BTC, {
        MainNet: config.node.BTC.WorkNodes[BTCWorkNodeIndex],
      });
    } else if (BTCWorkNodeIndex === config.node.BTC.WorkNodes.length) {
      Object.assign(config.node.BTC, {
        MainNet: config.originNode.BTC.MainNet,
      });
    }
    logger.error(
      `signData=${JSON.stringify({ address, to, value, fee, selector })}`,
      err,
      ['signBTCTx error'],
      address
    );
